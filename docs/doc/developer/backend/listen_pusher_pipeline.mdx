...
> Last updated: 2026-04-06 (STT provider abstraction refactor)
>
...
## 0. Provider Abstraction Notes

- `/v4/listen` now resolves its realtime STT path through `ProviderSelector` + `ProviderRegistry`.
- The current default path is still Deepgram, but the listen handler no longer depends on Deepgram-specific wiring directly.
- Transcript normalization is handled by a post-processing pipeline after provider output and before persistence.
- A separate finalize transcription service now exists for future high-accuracy reprocessing; conversation finalization still routes through pusher today.
...
sequenceDiagram
    ...
    Backend->>Backend: Resolve realtime provider (selector + registry)
    Backend->>Deepgram: Open STT WebSocket (default provider)
    ...
    loop Audio Streaming
        ...
        Backend->>Backend: Run transcript post-process pipeline
